...
const OrderDetails = () => {
  ...
  // finding the order
  const findedOrder = orders.find((order) => {
    ...
    <div className="p-4 flex flex-col gap-7 font-poppins">
      ...
      <div className="flex justify-between items-center px-2">
        <div className=" flex flex-col">
          <h1>#{orderid}</h1>
          <h1>{`${getActualDate(findedOrder.createdAt)} at ${getActualTime(
      ...
      >
        ...
        {findedOrder.orderStatus === "Pending" && (
          <div className=" flex justify-center items-center gap-2">
            <p>
              Your order is put on hold we appreciate your time don't worry if
              it is cancelled we refund your money shortly.
            </p>
          </div>
        )}
        {findedOrder.orderStatus === "Failed" && (
          <div className=" flex justify-center items-center gap-2">
            <p>
              Your Order is failed for some reason if the money debited we will
              refund with in 3-5 days
            </p>
          </div>
        )}
        {findedOrder.orderStatus === "Complete" && (
      ...
      <div className=" flex flex-col gap-2 px-3 ">
        {findedOrder.orderitems.map((item) => {
          return (
            <div
              key={item.id}
              className=" grid border-b py-6 border-gray-200"
            ...
            >
              <div className=" flex gap-2">
                ...
                <div className=" flex flex-col ">
                  <h1 className="text-base" style={{ fontWeight: "500" }}>
              ...
              <div className=" flex items-start justify-end">
                <h1 className=" text-xl font-mono">
                  &#8377;
                  {Math.round(
                    item.orderDetails.quantity * item.orderDetails.price
                  )}
                </h1>
              </div>
            </div>
      ...
      <div className=" px-3 flex flex-col gap-1 font-sans">
        ...
        <div className=" flex justify-between items-center text-green-500 text-sm ">
          ...
          <h1>&#8377;{findedOrder.totalAmount - findedOrder.finalAmount}</h1>
        </div>
